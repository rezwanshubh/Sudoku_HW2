...
if __name__ == '__main__':
    ...
    #broadcast connection to capture the server informationn
    ...
    while True:
        try:
            data = broadcastSocket.recv(1024) # Receiving the broadcast messge
            #print("Data is " + data)
            break
    ...
    server = (host, port) # server details
    sckt.connect(server) # connecting to the server with TCP
    print("Connected To Server")

    response = sckt.recv(1024) # waiting for server response

    #print (response)
    time.sleep(1)

    if response == 'Ok': # server says both the connections are ready

        client = Client(host) # RabbitMQ connection handling class
        client.setDaemon(True) # making it a deamon thread
        client.start()# staring the thread
        ...
        while (True):
            ...
            if ((len(value1) > 8 or len(value1) < 5) or (value1 == '') or (value1.isspace())):
                ...
                client.call(userName) #sending the user name to the server
                time.sleep(3)
                tk = Tk()
                board = SudokuBoard()
